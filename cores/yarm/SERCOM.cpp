...
{
  uint32_t clockId = 0, busId = SYSTEM_CLOCK_APB_APBC, pmMask = 0;
  IRQn_Type IdNvic=PendSV_IRQn ; // Dummy init to intercept potential error later

  if(sercom == SERCOM0)
  {
    clockId = SERCOM0_GCLK_ID_CORE;
    pmMask = MCLK_APBCMASK_SERCOM0;
    IdNvic = SERCOM0_IRQn;
  }
  else if(sercom == SERCOM1)
  {
    clockId = SERCOM1_GCLK_ID_CORE;
    pmMask = MCLK_APBCMASK_SERCOM1;
    IdNvic = SERCOM1_IRQn;
  }
  else if(sercom == SERCOM2)
  {
    clockId = SERCOM2_GCLK_ID_CORE;
    pmMask = MCLK_APBCMASK_SERCOM2;
    IdNvic = SERCOM2_IRQn;
  }
  else if(sercom == SERCOM3)
  {
    clockId = SERCOM3_GCLK_ID_CORE;
    pmMask = MCLK_APBCMASK_SERCOM3;
    IdNvic = SERCOM3_IRQn;
  }
  else if(sercom == SERCOM4)
  {
    clockId = SERCOM4_GCLK_ID_CORE;
    pmMask = MCLK_APBCMASK_SERCOM4;
    IdNvic = SERCOM4_IRQn;
  }
  else if(sercom == SERCOM5)
  {
    clockId = SERCOM5_GCLK_ID_CORE;
    busId = SYSTEM_CLOCK_APB_APBD;
    pmMask = MCLK_APBDMASK_SERCOM5;
    IdNvic = SERCOM5_IRQn;
  }
  ...
  //Setting clock
  GCLK->PCHCTRL[clockId].reg |= GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN_GCLK0; // Generic Clock Generator 0 is source
  ...
  switch (busId) {
    case SYSTEM_CLOCK_APB_APBC:
      MCLK->APBCMASK.reg |= pmMask;
      break;
    case SYSTEM_CLOCK_APB_APBD:
      MCLK->APBDMASK.reg |= pmMask;
      break;
  }

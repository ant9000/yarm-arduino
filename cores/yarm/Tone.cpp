...
#define TONE_TC         TC4
#define TONE_TC_IRQn    TC4_IRQn
#define TONE_TC_CLK     GCLK_TC4
#define TONE_TC_BUS     SYSTEM_CLOCK_APB_APBD
#define TONE_TC_MASK    MCLK_APBDMASK_TC4
#define TONE_TC_TOP     0xFFFF
#define TONE_TC_CHANNEL 0

void TC4_Handler (void) __attribute__ ((weak, alias("Tone_Handler")));
...
void tone (uint32_t outputPin, uint32_t frequency, uint32_t duration)
{
  ...
  if(!firstTimeRunning)
  {
    ...
    GCLK->PCHCTRL[TONE_TC_CLK].reg |= GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN_GCLK0;
    while (!(GCLK->PCHCTRL[TONE_TC_CLK].reg & GCLK_PCHCTRL_CHEN));

    switch (TONE_TC_BUS) {
        case SYSTEM_CLOCK_APB_APBC:
            MCLK->APBCMASK.reg |= TONE_TC_MASK;
            break;
        case SYSTEM_CLOCK_APB_APBD:
            MCLK->APBDMASK.reg |= TONE_TC_MASK;
            break;
    }
  }
  ...
  tmpReg |= TC_WAVE_WAVEGEN_MFRQ;   // Set TONE_TC mode as match frequency
